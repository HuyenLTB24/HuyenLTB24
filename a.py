...
class ImageProcessor:
    ...
    async def analyze_image(self, image_path: str) -> List[str]:
        ...
        try:
            log_message('info', f'Bắt đầu phân tích ảnh: {image_path}')
            ...
            log_message('error', f'Lỗi khi đọc và phân tích ảnh: {error}')
            return []
...
class TemplateManager:
    ...
    async def fetch_template(self, authorization: str, index: str, profile: dict, is_my_template: bool = True) -> Optional[dict]:
        ...
        endpoint = '/tournament/template/subscribe/my' if is_my_template else '/tournament/template/638403324'
        ...
        try:
            ...
            if template_data and template_data.get('url'):
                ...
                log_message('success', 
                    f'Đã tải ảnh template thành công cho {("My Template" if is_my_template else "Image Template")}.', 
                    index
                )
            ...
            # Thử endpoint dự phòng nếu không tìm thấy URL
            log_message('warning', 
                f'Không tìm thấy URL ảnh cho template {("My Template" if is_my_template else "Image Template")}. Thử lại với endpoint khác.', 
                f'Account {index}'
            )
            
            fallback_endpoint = 'https://notpx.app/api/v1/tournament/template/638403324'
            fallback_data = await self.api_client.make_api_request(
                fallback_endpoint, 'GET', authorization, index, profile
            )
            
            if fallback_data and fallback_data.get('url'):
                async with aiohttp.ClientSession() as session:
                    async with session.get(fallback_data['url']) as response:
                        image_data = await response.read()
                        
                log_message('success', 'Đã tải ảnh template thành công từ endpoint dự phòng.', index)
            ...
            raise ValueError('Không tìm thấy URL ảnh cho template từ endpoint dự phòng.')
        ...
        try:
            ...
            if user_data:
                ...
                if self.want_repaint:
                    ...
                    if template_data and template_data['image_data']:
                        image_path = f'image_{profile["name"]}.png'
                        with open(image_path, 'wb') as f:
                            f.write(template_data['image_data'])
